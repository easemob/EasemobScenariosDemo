...
import com.hyphenate.scenarios.common.helper.DeveloperModeHelper
import com.hyphenate.scenarios.databinding.DemoActivityMeInformationBinding
import com.hyphenate.scenarios.interfaces.IProfileResultView
import com.hyphenate.scenarios.utils.CameraAndCropFileUtils
...
class UserInformationActivity:EaseBaseActivity<DemoActivityMeInformationBinding>(),
    View.OnClickListener, IProfileResultView {
        ...
        showSelectDialog = SimpleListSheetDialog(
            ...
            itemList = mutableListOf(
                EaseMenuItem(
                    ...
                    title = getString(R.string.em_self_information_camera),
                ...
                EaseMenuItem(
                    ...
                    title = getString(R.string.em_self_information_picture),
                    titleColor = ContextCompat.getColor(context, com.hyphenate.easeui.R.color.ease_color_primary)
                )
            ),
    ...
    override fun updateNickNameSuccess(user: EaseProfile?) {
        user?.let {
    ...
    override fun updateNickNameFail(code: Int, error: String) {
        mContext.showToast("updateUsername error $code $error")
    }

    override fun uploadAvatarSuccess() {
        binding.ivAvatar.setImageURI(imageUri)
        updateUserAvatar()
    }

    override fun uploadAvatarFail(code: Int, error: String) {
        mContext.showToast("uploadFile error $code $error")
    }
       ...
       when(v?.id){
           R.id.avatar_layout -> {
               if (DeveloperModeHelper.isRequestToAppServer()){
                   ...
                   mainScope().launch {
                       mContext.showToast(mContext.getString(R.string.em_self_information_checked_model))
                   }
               }
           }
