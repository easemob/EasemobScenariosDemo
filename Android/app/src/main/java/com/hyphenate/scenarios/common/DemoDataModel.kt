package com.hyphenate.scenarios.common
...
import com.hyphenate.scenarios.common.room.AppDatabase
...
import com.hyphenate.scenarios.common.room.extensions.parseToDbBean
...
class DemoDataModel(private val context: Context) {
    ...
    private val matchUserList = ConcurrentHashMap<String, MatchUserInfo>()
    ...
    fun insertMatchUsersInfo(userIds: List<MatchUserInfo>){
        if (userIds.isNotEmpty()) {
            userIds.map { info->
                info.id.let {
                    matchUserList[it] = info
                }
            }
        }
    }

    fun updateMatchUserInfo(info:MatchUserInfo){
        info.id.let {
            matchUserList[it] = info
        }
    }

    fun getMatchUserInfo(id:String):MatchUserInfo?{
        if (matchUserList.containsKey(id)){
           return matchUserList[id]
        }
        return null
    }
    ...
    fun setCurrentMatchInfo(json:String?){
        PreferenceManager.putValue(KEY_CURRENT_MATCH_INFO,json?:"")
    }

    fun getCurrentMatchInfo():String{
        return PreferenceManager.getValue(KEY_CURRENT_MATCH_INFO,"")
    }

    private fun loadSelf(){
        val json = getCurrentMatchInfo()
        if (json.isNotEmpty()){
            val `object` = JSONObject(json)
            if (`object`.has("chatUserName")){
                val username = `object`.getString("chatUserName")
                val match = MatchUserInfo(username)
                if (`object`.has("phoneNumber")){
                    match.matchedUser = `object`.getString("phoneNumber")
                }
                if (`object`.has("agoraUid")){
                    match.agoraUid = `object`.getString("agoraUid")
                }
                if (`object`.has("avatarUrl")){
                    match.avatar = `object`.getString("avatarUrl")
                }
                DemoHelper.getInstance().dataModel.updateMatchUserInfo(match)
            }
        }
    }
    ...
    fun setExceptionCancelMatch(isNormal:Boolean){
        val key = getCancelMatchKey()
        if (key.isNotEmpty()){
            PreferenceManager.putValue(key,isNormal)
        }
    }

    fun isExceptionCancelMatch():Boolean{
        val key = getCancelMatchKey()
        return PreferenceManager.getValue(key,true)
    }

    private fun getCancelMatchKey():String{
        if (EaseIM.isInited()){
            val self = ChatClient.getInstance().currentUser
            val appKey = BuildConfig.APPKEY
            if (self.isNotEmpty() && appKey.isNotEmpty()){
                return "${appKey}_${self}_${KEY_CANCEL_MATCH}"
            }
        }
        return ""
    }
    ...
    companion object {
        ...
        private const val KEY_CURRENT_MATCH_INFO = "key_current_match_info_model"
        private const val KEY_CANCEL_MATCH = "key_cancel_match"
    }
